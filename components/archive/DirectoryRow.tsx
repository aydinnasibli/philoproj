...
export default function DirectoryRow({ philosopher }: { philosopher: PhilosopherListItem }) {
  ...
  return (
    <Link href={`/philosophers/${philosopher.slug}`} className="no-underline group">
      <div className={`grid grid-cols-[1fr] md:grid-cols-[1fr_200px_200px] items-center cursor-pointer border-b border-border-pale bg-transparent transition-[background-color,box-shadow] duration-150 group-hover:bg-[rgba(139,115,85,0.04)] ${hoverShadow} py-3.5`}>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="translate-x-0 transition-transform duration-180 group-hover:translate-x-1">
            ...
            {(philosopher.birthYear || philosopher.deathYear) && (
              <span className="font-sans text-2xs text-ink-muted block mt-px">
        ...
        <span className="hidden md:inline font-sans text-2xs font-semibold tracking-[0.08em] uppercase text-ink-muted transition-colors duration-150 group-hover:text-accent">
