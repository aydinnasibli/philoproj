...
const SCHOOL_META: Record<string, { title: string; tagline: string; description: string }> = {
  "school-socratic-method": { title: "The Socratic Method", tagline: "Dialectic & Examination", description: "You are a relentless questioner. Where others accept received wisdom, you probe assumptions through dialogue. Truth, for you, is not handed down — it is drawn out through rigorous cross-examination, and the unexamined life is simply not worth living." },
...
export default function QuizOverlay({ onClose, onResult }: Props) {
  ...
  return (
    ...
    >
      {/* Ruled lines texture */}
      <div className="absolute inset-0 pointer-events-none bg-[repeating-linear-gradient(0deg,transparent,transparent_47px,color-mix(in_srgb,var(--color-zinc-600)_2%,transparent)_48px)]" />
        ...
        <AnimatePresence mode="wait">
          ...
          {!isFinished ? (
            <motion.div key={step} initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -18 }} transition={{ duration: 0.38, ease: [0.22, 1, 0.36, 1] }}>
              ...
              <div className="h-px bg-linear-to-r from-zinc-600/14 to-transparent mb-7" />
