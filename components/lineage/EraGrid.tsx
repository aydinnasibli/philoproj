...
function DashboardCard({ era }: { era: EraWithPhilosophers }) {
  return (
    <div className="p-10 h-full flex flex-col gap-5 relative overflow-hidden min-h-[300px] group">
      ...
      <span className="font-sans text-3xs font-semibold tracking-[0.2em] uppercase text-accent">
        {formatYear(era.startYear)} — {formatYear(era.endYear)}
      </span>
      <h2 className="font-serif text-[1.75rem] font-medium leading-[1.15] text-ink">{era.title}</h2>
      <p className="font-sans text-sm leading-[1.75] text-ink-muted flex-1">{era.description}</p>

      {era.philosophers.length > 0 && (
        <div>
          <p className="font-sans text-3xs tracking-[0.15em] uppercase text-ink-muted mb-2.5 font-semibold">Notable Thinkers</p>
          <div className="flex flex-wrap gap-1.5">
            {era.philosophers.map((p) => (
              <Link
                key={p._id}
                href={`/philosophers/${p.slug}`}
                className="flex items-center gap-1.5 px-2.5 py-1 border border-border text-xs font-sans text-ink no-underline bg-transparent transition-[border-color,color] duration-200 hover:border-accent hover:text-accent"
              >
                {p.avatarUrl && (
