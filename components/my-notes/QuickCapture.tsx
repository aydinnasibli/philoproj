    ...
    <div onClick={e => { if (e.target === e.currentTarget) onClose(); }} className="fixed inset-0 z-600 bg-[rgba(18,15,10,.6)] backdrop-blur-sm flex items-center justify-center p-6">
      <div className="w-full max-w-[560px] bg-(--mn-card) rounded-[4px] border border-(--mn-border) shadow-[0_30px_80px_rgba(0,0,0,.22)] overflow-hidden">
        ...
        <div className="px-[22px] pt-4">
          <input value={title} onChange={e => setTitle(e.target.value)} placeholder="Title (optional)…" className="w-full bg-transparent border-none outline-none font-cinzel text-sm tracking-[.04em] text-(--mn-ink) mb-3" />
        ...
        <div className="px-[22px] py-2.5 border-t border-(--mn-border) flex items-center justify-between bg-(--mn-surface)">
          <span className="text-[11.5px] text-(--mn-ink-3) italic">⌘↵ save · Esc cancel</span>
          <div className="flex gap-2">
            <button onClick={onClose} className="bg-transparent border border-(--mn-border) text-(--mn-ink-3) px-[14px] py-1 text-3xs font-cinzel cursor-pointer rounded-[2px]">Cancel</button>
            <button onClick={save} className="bg-(--mn-ink) text-(--mn-surface) border-none px-[18px] py-1 text-3xs font-cinzel tracking-widest cursor-pointer rounded-[2px]">Save</button>
          </div>
        </div>
      </div>
    </div>
