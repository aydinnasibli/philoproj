        ...
        return (
          <div key={n.id} onClick={() => onOpen(n.id)} className="flex gap-5 pb-[26px] mb-[26px] border-b border-(--mn-border) cursor-pointer transition-opacity duration-150 hover:opacity-[.78]">
            <div className="w-[50px] shrink-0 pt-1 text-right">
              {n.pinned && (
                <div className="ml-auto mb-1.5 size-2 rounded-full bg-(--mn-gold) shadow-sm flex items-center justify-center">
                  <div className="size-0.5 rounded-full bg-white/40 -translate-x-px -translate-y-px" />
                </div>
              )}
              <div className="font-cinzel text-[9.5px] text-(--mn-ink-3) leading-normal">
                {new Date(n.createdAt).toLocaleDateString("en-GB", { day: "numeric", month: "short" })}
              </div>
              <div className="text-4xs text-(--mn-border2) mt-0.5">{new Date(n.createdAt).getFullYear()}</div>
            </div>
            <div className="flex-1 min-w-0">
              ...
              <div className="flex gap-[5px] mt-[9px] flex-wrap">
                {(n.tags ?? []).map(tag => (
                  <span key={tag} className={`text-[8.5px] font-cinzel px-1.5 py-px rounded-[2px] border ${tagStyle(tag, tags).pill}`}>{tag}</span>
                ))}
              </div>
            </div>
          </div>
